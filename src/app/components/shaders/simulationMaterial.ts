...
function getPoint(v: THREE.Vector4, size: number, data: Float32Array, offset: number): Float32Array {
  v.set(Math.random() * 2 - 1, Math.random() * 2 - 1, Math.random() * 2 - 1, 0)
  if (v.length() > 1) return getPoint(v, size, data, offset)
  return v.normalize().multiplyScalar(size).toArray(data, offset) as unknown as Float32Array
}
